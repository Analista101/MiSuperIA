...
st.set_page_config(
    ...
    page_icon="https://img.icons8.com/neon/256/iron-man.png", # Restaurado Icono Iron Man
    layout="wide"
)
...
# PERSONALIDAD JARVIS
PERSONALIDAD = f"Eres JARVIS, el asistente de la Srta. Diana. Tu tono es sofisticado, ingenioso y servicial. Santiago, Chile. {fecha_actual}."

# --- 2. ESTILOS VISUALES STARK (RESTAURADOS) ---
st.markdown("""
    <style>
    .stApp { 
        background: #010409; 
        background-image: radial-gradient(circle at 50% 30%, rgba(0, 242, 255, 0.15) 0% , transparent 60%), 
        url('https://wallpaperaccess.com/full/156094.jpg'); 
...
with tabs[1]:
    ...
    f = st.file_uploader("Cargar evidencia técnica", type=['pdf','docx','png','jpg','xlsx'])
        ...
        with st.spinner("Procesando con Llama-4-Scout..."):
            try:
                if f.type in ["image/png", "image/jpeg"]:
                    ...
                    res = client.chat.completions.create(model=MODELO_VISION_SCOUT, messages=[{"role": "user", "content": [{"type": "text", "text": "Analiza detalladamente esta imagen técnico-industrial."}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64}"}}]}])
                    out = res.choices[0].message.content
                elif f.name.endswith('.xlsx'):
                    df = pd.read_excel(f)
                    out = f"Análisis de {len(df)} filas. Columnas: {list(df.columns)}\n\n{df.describe(include='all').to_string()}"
                    ...
                    out = res.choices[0].message.content
                else: out = "Archivo procesado."
                st.markdown(out)
                st.download_button("📥 DESCARGAR REPORTE", generar_pdf_reporte("ANÁLISIS SCOUT", out), "Reporte_Stark.pdf")
...
with tabs[2]:
    ...
    asunto = st.text_input("Asunto:", value="REPORTE DE MISIÓN")
    ...
    if st.button("🚀 TRANSMITIR"):
        try:
            ...
            srv.send_message(msg); srv.quit(); st.success("Enviado con éxito.")
        except Exception as e: st.error(f"Error: {e}")

# --- TAB 3: LABORATORIO (CON FILTROS) ---
with tabs[3]:
    st.subheader("🎨 Forja de Prototipos")
    idea = st.text_input("Concepto Visual:"); estilo = st.selectbox("Renderizado:", ["Cinematic Marvel", "Digital Blueprint", "Cyberpunk", "Photorealistic"])
